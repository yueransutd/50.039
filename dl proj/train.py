        ...
        for i, (images, captions, lengths) in enumerate(data_loader):
            ...
            if i % args.log_step == 0:
                print('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, Perplexity: {:5.4f}, Accuracy: {:.4f}'
                      .format(epoch, args.num_epochs, i, total_step, loss.item(), np.exp(loss.item()), accuracy/float(i+1))) 
                with open('train_loss_lemma.txt', 'a') as fi:
                    fi.write('\n'+'epoch = {}, i = {}, tr_loss = {}, acc = {}'.format(epoch+1, i+1, loss.item(), accuracy/float(i+1)))

            # Save the model checkpoints
            if (i+1) % args.save_step == 0:
                torch.save(decoder.state_dict(), os.path.join(
                    args.model_path, 'decoder-{}-{}-lemma.ckpt'.format(epoch+1, i+1)))
                torch.save(encoder.state_dict(), os.path.join(
                    args.model_path, 'encoder-{}-{}-lemma.ckpt'.format(epoch+1, i+1)))
